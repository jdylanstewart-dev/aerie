...
public final class PostgresResultsCellRepository implements ResultsCellRepository {
  ...
  private static void postResults(
    ...
    try (
        final var postGoalSatisfactionAction = new PostGoalSatisfactionAction(connection);
        final var postCreatedActivitiesAction = new PostCreatedActivitiesAction(connection);
        final var postSatisfyingActivitiesAction = new PostSatisfyingActivitiesAction(connection)
    ) {
      postGoalSatisfactionAction.apply(analysisId, goalSatisfaction);
      postCreatedActivitiesAction.apply(analysisId, createdActivities);
      postSatisfyingActivitiesAction.apply(analysisId, satisfyingActivities);
    }
  }
